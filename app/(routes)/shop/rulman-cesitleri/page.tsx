...
import { rulmanProducts, ProductType } from '@/constans'
import { useFavorites } from '@/app/context/FavoritesContext'
import { toast } from 'sonner'

const RulmanCesitleriPage = () => {
  ...
  const productTypes: Record<string, (product: ProductType) => boolean> = {
    'Tümü': () => true,
    'Bilyalı': (product: ProductType) => product.title.includes('Bilyalı') || product.description.includes('bilyalı'),
    'Makaralı': (product: ProductType) => product.title.includes('Makaralı') || product.description.includes('makaralı'),
    'İğneli': (product: ProductType) => product.title.includes('İğneli') || product.description.includes('iğneli'),
    'Konik': (product: ProductType) => product.title.includes('Konik') || product.description.includes('konik'),
  ...
  const handleFavoriteClick = (e: React.MouseEvent, product: ProductType) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    <div className="min-h-screen bg-[#1a1f2b] py-12 pt-28">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <Image 
            src="/slider/rulman-banner.jpg" 
            alt="Rulman Çeşitleri" 
            fill 
          ...
          <div className="absolute inset-0 z-20 flex flex-col justify-center items-center text-center p-8">
            ...
            >
              ...
              >
                Değirmen ve endüstriyel tesisler için çeşitli rulman ürünleri ve çözümleri
              </motion.p>
            </motion.div>
          </div>
        </motion.div>
        ...
        >
          <h2 className="text-2xl font-semibold mb-4">Rulman Çeşitleri ve Kullanım Alanları</h2>
          <p className="mb-4">
            Rulmanlar, makine parçaları arasındaki sürtünmeyi azaltarak daha verimli çalışma sağlayan hayati mekanik bileşenlerdir. Endüstriyel uygulamalarda, özellikle değirmen ekipmanlarında, rulmanlar uzun ömürlü ve güvenilir performans için kritik öneme sahiptir.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            <div className="bg-white/5 rounded-xl p-4">
              <h3 className="text-xl font-semibold text-blue-400 mb-2">Yüksek Hassasiyet</h3>
              <p>Hassas üretim süreçleriyle imal edilen rulmanlar, yüksek verim ve minimum sürtünme kaybı sağlar.</p>
            </div>
            <div className="bg-white/5 rounded-xl p-4">
              <h3 className="text-xl font-semibold text-blue-400 mb-2">Uzun Ömür</h3>
              <p>Kaliteli malzemelerden üretilen rulmanlar, zorlu koşullarda bile uzun süre sorunsuz çalışabilir.</p>
            </div>
            <div className="bg-white/5 rounded-xl p-4">
              <h3 className="text-xl font-semibold text-blue-400 mb-2">Çeşitli Uygulamalar</h3>
              <p>Farklı tiplerdeki rulmanlar, değirmenlerde, konveyör sistemlerinde, motorlarda ve daha birçok endüstriyel uygulamada kullanılabilir.</p>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div 
          ...
          initial="hidden"
          animate="visible"
          className="hidden md:flex flex-wrap justify-center gap-3 mb-12 relative z-30"
        >
          {filters.map((filter) => (
            <motion.button
              key={filter}
            ...
            currentItems.map((product) => (
              ...
              >
                  ...
                  <div className="absolute top-4 right-4 z-10">
                    <motion.button 
                      ...
                      className={`p-2 rounded-full ${
                        isFavorite(product.id.toString()) 
                          ? 'bg-red-500/80' 
                          : 'bg-white/10'
                      } backdrop-blur-md hover:bg-white/20 transition-colors duration-200`}
                    ...
                    >
                      <svg 
                        ...
                        fill={isFavorite(product.id.toString()) ? "currentColor" : "none"} 
                        viewBox="0 0 24 24" 
                        stroke="currentColor"
                      >
                        <path 
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  <p className="text-gray-300 mb-4 flex-1 text-sm">{product.description}</p>
        ...
        >
          ...
          <div className="flex flex-wrap justify-center items-center gap-8">
            {['/slider/skf.png', '/slider/fag.png', '/slider/ntn.png', '/slider/nsk.png', '/slider/best.png'].map((brand, index) => (
              <motion.div
                key={index}
...
export default RulmanCesitleriPage 