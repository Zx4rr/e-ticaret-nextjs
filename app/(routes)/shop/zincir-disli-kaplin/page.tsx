...
import { zincirDisliKaplinProducts, ProductType } from '@/constans'
...
const ZincirDisliKaplinPage = () => {
  ...
  // Zincir, dişli ve kaplin ürünlerini direkt kullan
  const productsList = zincirDisliKaplinProducts

  // Create a map of product types for filtering
  const productTypes: Record<string, (product: ProductType) => boolean> = {
    'Tümü': () => true,
    'Zincirler': (product: ProductType) => product.title.includes('Zincir') || product.description.includes('zincir'),
    'Dişliler': (product: ProductType) => product.title.includes('Dişli') || product.description.includes('dişli'),
    'Kaplinler': (product: ProductType) => product.title.includes('Kaplin') || product.description.includes('kaplin'),
    'Zincir Dişlileri': (product: ProductType) => (product.title.includes('Zincir') && product.description.includes('dişli')) || (product.title.includes('Dişli') && product.description.includes('zincir')),
    ...
    <div className="min-h-screen bg-[#1a1f2b] py-12 pt-28">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <Image 
            src="/slider/zincir-banner.png" 
            alt="Zincir Dişli Kaplin" 
            fill 
          ...
          <div className="absolute inset-0 z-20 flex flex-col justify-center items-center text-center p-8">
            ...
            >
              ...
              >
                Endüstriyel zincir, dişli ve kaplin çeşitleri ile değirmen endüstrisine yönelik çözümler
              </motion.p>
            </motion.div>
          </div>
        </motion.div>
        ...
        >
          <h2 className="text-2xl font-semibold mb-4">Zincir, Dişli ve Kaplin Sistemleri</h2>
          <p className="mb-4">
            Zincir, dişli ve kaplin sistemleri, endüstriyel ekipmanlarda hareket ve güç aktarımı için hayati öneme sahip mekanik bileşenlerdir. Bu ürünler, farklı makine parçaları arasında verimli güç iletimi sağlayarak üretim hatlarının güvenilir çalışmasına katkıda bulunur.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            <div className="bg-white/5 rounded-xl p-4">
              <h3 className="text-xl font-semibold text-blue-400 mb-2">Yüksek Dayanım</h3>
              <p>Zorlu endüstriyel koşullarda uzun ömürlü ve sorunsuz çalışma için üstün malzeme kalitesi ve işçilik.</p>
            </div>
            <div className="bg-white/5 rounded-xl p-4">
              <h3 className="text-xl font-semibold text-blue-400 mb-2">Hassas Mühendislik</h3>
              <p>Optimum performans ve minimum sürtünme kaybı için hassas ölçülerde tasarlanmış ürünler.</p>
            </div>
            <div className="bg-white/5 rounded-xl p-4">
              <h3 className="text-xl font-semibold text-blue-400 mb-2">Özel Çözümler</h3>
              <p>Standart ürünlerin yanında, özel uygulamalar için özelleştirilmiş zincir, dişli ve kaplin çözümleri.</p>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div 
          ...
          initial="hidden"
          animate="visible"
          className="hidden md:flex flex-wrap justify-center gap-3 mb-12 relative z-20"
        >
          {filters.map((filter) => (
            <motion.button
              key={filter}
...
export default ZincirDisliKaplinPage 